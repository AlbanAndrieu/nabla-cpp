...
def WriteEclipseProjectFile(env, cproject, tree):
    ...
    env.Execute("sed 's/<cproject/<?fileVersion 4.0.0?><cproject/' < " + cproject+".new > " + cproject)
    ...
    if os.path.exists(cproject):
        ...
        for config in options:
            ...
            for includePath in includePaths[1:]:
                ...
                if '.' == includePath:
                    continue
                etree.SubElement(config, 'listOptionValue', {'value': includePath, 'bulitIn': 'false', 'kplus_generated': 'true'})
...
#    print 'Add target:', targetName, targetArguments, targetDescription
    targetElement = etree.SubElement(targetsElement, 'target', {'description': targetDescription, 'targetID': 'ch.hsr.ifs.sconsolidator.Builder'} )
...
def AddTargets2EclipseProject(env, progDirs, clean=False):
    ...
    if os.path.exists(cproject):
        ...
        sconsolidator = tree.find(".//storageModule[@moduleId='ch.hsr.ifs.sconsolidator.core.buildtargets']")

        if sconsolidator is None: # is not None:
            sconsolidator = etree.SubElement(tree.getroot(), 'storageModule', {'moduleId': 'ch.hsr.ifs.sconsolidator.core.buildtargets'})
        ...
        env.AddSconsTarget2Eclipse(etree, targets, 'Eclipse', 'action=includes', '#.Eclipse includes')
        env.AddSconsTarget2Eclipse(etree, targets, 'Eclipse', 'action=targets', '#.Eclipse targets')
        env.AddSconsTarget2Eclipse(etree, targets, 'Eclipse', 'action=clean_targets', '#.Remove Eclipse targets')
        env.AddSconsTarget2Eclipse(etree, targets, 'print_libpath', '-s', '#.Print LD_LIBRARY_PATH')
        env.AddSconsTarget2Eclipse(etree, targets, 'print_libpath', '-s --install', '#.Print LD_LIBRARY_PATH (kplushome3)')

        for module in progDirs:
            env.AddSconsTarget2Eclipse(etree, targets, str(module), '--install')
