...
#------------------------------------------------------------------------------
colorPatterns = [
    (re.compile(r'(.*: [Ww]arning[:,].*)')                            , r'%s[Warning] %s\1%s' % (cb_red, cb_yellow, c_norm)),
    (re.compile(r'(.*: [Ee]rror[:,].*)')                              , r'%s[Error] \1%s'     % (cb_red, c_norm)),
    (re.compile(r'\[(CC|CXX|UIC|MOC|Q2K|RCC|RAN|LNK)\](.*?)([^/]+)$') , r'%s[\1]%s\2%s\3%s'   % (c_blue, c_purple, cb_purple, c_norm)),
    (re.compile(r'\[(INST)\](.*)$')                                   , r'%s[\1]%s\2%s'       % (cb_green, c_green, c_norm)),
    # verbose mode
    (re.compile(r'(Install file:)(.*)$')                              , r'%s[\1]%s\2%s'       % (cb_green, c_green, c_norm)),
    (re.compile(r'^(ranlib|Removed)(.*)$')                            , r'%s\1%s\2'           % (cb_cyan, c_norm)),
    (re.compile(r'^(CC|cc|gcc|g\+\+|ar) ([^\s]+) ([^\s]+)(.*)$')      , r'%s\1%s \2 %s\3%s\4' % (cb_cyan, c_norm, c_cyan, c_norm)),
]
