...
if os.name == 'nt':
    ...
    def Lock(lock_file):
        ...
        LockFileEx(hfile, LOCK_EX, 0, 0, 0xFFFF0000, ctypes.byref(OVERLAPPED()))
        try:
            yield
        finally:
...
class ThirdParty:
    ...
    def __init__(self, arch, serial, base_url, base_url_old, sandbox_dir, third_parties_dir):
        ...
        self._cache_dir   = '%s/%s-cache%s' % (os.getenv('WORKSPACE', tempfile.gettempdir()), os.path.splitext(os.path.basename(__file__))[0], user_str)
    ...
    def expandLatestKeywords(self):
            ...
            with contextlib.closing(urllib.urlopen('%s/%s/?C=N;O=D'%(self._base_url, name))) as page:
                for line in page:
                    ...
                    if match:
                        ...
                        break
            if version == 'latest':
                raise SystemExit('Couldn\'t find any existing version for %s' % self._serial)
        ...
        if digest == 'latest':
            ...
            with contextlib.closing(urllib.urlopen('%s/%s/%s/%s/?C=M;O=D'%(self._base_url, name, version, pkg_arch))) as page:
                for line in page:
                    match = re.search(r'href="([a-f0-9]{32})".*(\d\d-\w\w\w-20\d\d \d\d:\d\d)', line)
                    if match:
                        isExpanded = True
                        timestamp = time.mktime(time.strptime(match.group(2), '%d-%b-%Y %H:%M'))
                        digest = match.group(1)
                        break
            if not timestamp:
                raise SystemExit('Couldn\'t find any matching artifact for %s' % self._serial)
    ...
    def download(self):
        ...
        if not os.path.isdir(self._cache_dir): os.mkdir(self._cache_dir)
        unchecked_dir = '%s/unchecked' % (self._cache_dir)
        if not os.path.isdir(unchecked_dir): os.mkdir(unchecked_dir)
        ...
        download_count = 0
        try_number = 3
        pkg           = '%s/%s-%s-%s-%s'%(self._cache_dir, name, version, pkg_arch, digest)
        #pkg           = '%s/%s-%s-%s'%(self._cache_dir, name, version, digest)
        unchecked_pkg = '%s/%s-%s-%s-%s'%(unchecked_dir,   name, version, pkg_arch, digest)
        ...
        while not download_ok:
            ...
            for base_url in base_urls:
                ...
                name, header = urllib.urlretrieve('%s/%s'%(base_url, self._serial), unchecked_pkg)
                no_error = True
                if (scheme != 'file') and ('Content-Type' in header): # There is no Content-Type when the 3rd party exists
            ...
            if not download_ok and download_count >= try_number:
                print '-> ABORT'
                raise SystemExit('Tried to downloaded package %s %s %d times without success' %(name, version, try_number))
    ...
    def get(self):
        ...
        # Create directories if needed
        if not os.path.isdir(thirdPartyRoot): os.makedirs(thirdPartyRoot)
        ...
        if(self._arch=='winnt' or self._arch=='win'):
            ...
            subprocess.check_call([tar_cmd(), '-y', '-o%s'%(pkg_dir), 'e', pkg], shell=False)
            pkg = '%s/*'%(pkg_dir)
            subprocess.check_call([tar_cmd(), '-y', 'x', pkg], shell=False)
        else:
            subprocess.check_call([tar_cmd(), '-x', '-z', '-f', pkg], shell=False)
        print '-> create serial',
        ...
        with open(bom) as fic:
            for line in fic:
                if re.match(r'^\s*#', line):
                    ...
                    print 'arch : %s'%(arch),
                    match = re.search(r'/(%s|noarch)(-[^_]+)?(_[^/]+)?/'%arch, line)
                    if match:
                        #serialCompiler = match.group(1)
                        targetArch = match.group(1)
                        print '(match ) targetArch : %s'%(targetArch),
                        serialCompiler = match.group(2)[1:] if match.group(2) else None
                        print '- serialCompiler : %s'%(serialCompiler),
                        serialDetails  = match.group(3).split('_')[1:] if match.group(3) else None
...
def writeArchBom(serials):
    if not os.path.isdir(thirdPartyRoot): os.mkdir(thirdPartyRoot)
...
def main(args):
    ...
    parser.add_argument('--arch',     choices=['lin', 'sol', 'solsparc', 'win', 'x86sol', 'x86Linux', 'sun4sol', 'winnt'], help = 'arch on which the build is done', required=True)
    parser.add_argument('--bit',      default='64', choices=['32','64'], help = '32 or 64 bit', required=False)
    parser.add_argument('--compiler', default='', help = 'compiler name and version (gcc4, st12, vc11, ...)', required=False)
    parser.add_argument('--base_url_old', default='http://home.nabla.mobi:7072/download/cpp-old/', help = 'Base URL where to download third parties from. Use file:// for using a local ESCROW repository', required=False)
    parser.add_argument('--base_url', default='http://home.nabla.mobi:7072/download/cpp/', help = 'Base URL where to download third parties from. Use file:// for using a local ESCROW repository', required=False)
    parser.add_argument('--bom',      default=None, help = 'file listing serials of third parties to download', required=True)
    parser.add_argument('--third_parties_dir', default='3rdparties', help = 'directory where third parties will be stored', required=False)
    parser.add_argument('--serials',  action='append', help = 'directly specify some serials (multiple uses are possible', required=False)
    ...
    download(args.arch, args.bit, args.compiler, args.base_url_old, args.base_url, args.bom, args.third_parties_dir, args.serials)
