...
class colorizer:
    ...
    def __init__(self, noColors=False):
        ...
        # C compile messages
        self.cmpCStaticMsg = self.message(
            '$CCCOM', 'Compiling',
            self.cBlue, self.cPurple, self.cYellow, self.cEnd,
        )
        self.cmpCSharedMsg = self.message(
            '$SHCCCOM', 'Compiling',
            self.cBlue, self.cPurple, self.cYellow, self.cEnd,
        )

        # C++ compile messages
        self.cmpCPPStaticMsg = self.message(
            '$CXXCOM', 'Compiling',
            self.cBlue, self.cPurple, self.cYellow, self.cEnd,
        )
        self.cmpCPPSharedMsg = self.message(
            '$SHCXXCOM', 'Compiling',
            self.cBlue, self.cPurple, self.cYellow, self.cEnd,
        )

        # Linking messages
        self.lnkStaticMsg = self.message(
            '$ARCOM', 'Linking library',
            self.cRed, self.cPurple, self.cYellow, self.cEnd, True,
        )
        self.lnkSharedMsg = self.message(
            '$SHLINKCOM', 'Linking shared object',
            self.cRed, self.cPurple, self.cYellow, self.cEnd, True,
        )
        self.lnkProgramMsg = self.message(
            '$LINKCOM', 'Linking program',
            self.cRed, self.cPurple, self.cYellow, self.cEnd, True,
        )
    ...
    def colorizeBuilder(self, env, builder, text='', target=True, color=''):
        ...
        if builder == 'SharedLibrary' or builder == 'LoadableModule':
            ...
            action = self.message(
                _action, text, color, self.cPurple, self.cYellow, self.cEnd, target,
            ).getAction()
...
def generate(env, **kw):
    ...
    if env['color']:
        # from colorizer-V1 import colorizer
        col = colorizer()
        col.colorize(env)
