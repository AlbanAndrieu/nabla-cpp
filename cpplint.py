...
class _CppLintState(object):
    ...
    def SetProjectName(self, project_name):
        """Sets the project name for header guard creation."""
...
def GetHeaderGuardCPPVariable(filename):
  ...
  """
    ...
    return prefix + re.sub(r'[-./\s]', '_', fileinfo.RepositoryName()).upper() + '_'
...
def CheckForHeaderGuard(filename, lines, error):
    ...
    if endif != ('#endif  // %s' % cppvar):
        ...
        error(
            filename, endif_linenum, 'build/header_guard', error_level,
...
class _ClassState(object):
    ...
    def CheckFinished(self, filename, error):
        ...
        if self.classinfo_stack:
            ...
            error(
                filename, self.classinfo_stack[0].linenum, 'build/class', 5,
...
def CheckForNonStandardConstructs(
    ...
    if brace_depth <= 0:
        ...
        if ((classinfo.virtual_method_linenumber is not None) and
            ...
            error(
                filename, classinfo.linenum, 'runtime/virtual', 4,
...
def CheckSpacing(filename, clean_lines, linenum, error):
    ...
    if IsBlankLine(line):
        ...
        if (
            prevbrace != -1 and prev_line[prevbrace:].find('}') == -1
...
def CheckStyle(filename, clean_lines, linenum, file_extension, error):
    ...
    elif not initial_spaces and line[:2] != '//' and Search(
        r'[^:]:\s*$',
        line,
    ):
        error(
            filename, linenum, 'whitespace/labels', 4,
            'Labels should always be indented at least one space.  '
    ...
    if (
        ...
        if line_width > 100:
            error(
                filename, linenum, 'whitespace/line_length', 4,
...
def CheckLanguage(
    filename, clean_lines, linenum, file_extension, include_state,
    error,
):
    ...
    if match:
        ...
        if Match(r'(f|ind|io|i|o|parse|pf|stdio|str|)?stream$', include):
            ...
            if not _IsTestFilename(filename):
                error(
                    filename, linenum, 'readability/streams', 3,
    ...
    CheckCStyleCast(
        filename, linenum, line, clean_lines.raw_lines[linenum],
        'static_cast',
    ...
    if Search(r'\bdynamic_cast<', line) and not _IsTestFilename(filename):
        error(
            filename, linenum, 'runtime/rtti', 5,
    ...
    if Search(r'\bsscanf\b', line):
        error(
            filename, linenum, 'runtime/printf', 1,
    ...
    if match and linenum + 1 < clean_lines.NumLines():
        ...
        if not Search(r'^\s*};', next_line):
            error(
                filename, linenum, 'readability/constructors', 3,
...
def CheckCStyleCast(
    ...
    if sizeof_match:
        error(
            filename, linenum, 'runtime/sizeof', 1,
    ...
    if function_match:
        if (
            ...
            error(
                filename, linenum, 'readability/function', 3,
...
for _template in (
    'copy', 'max', 'min', 'min_element', 'sort', 'swap',
    'transform',
):
...
def ProcessFile(filename, vlevel):
    ...
    if (
        filename != '-' and file_extension != 'cc' and file_extension != 'h'
        ...
        if carriage_return_found and os.linesep != '\r\n':
            ...
            # several lines.
            Error(
                filename, 0, 'whitespace/newline', 1,
