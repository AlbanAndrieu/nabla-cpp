...
circular_queueThread::circular_queueThread()
    : m_stoprequested(false), m_nbSteps(0), m_type(true) {}
...
void circular_queueThread::go() {
  ...
  m_thread = boost::shared_ptr<boost::thread>(
      new boost::thread(boost::bind(&circular_queueThread::work, this)));
}
...
void circular_queueThread::setType(bool type) { m_type = type; }
