 ...
 *
 * A thread safe circular queue - a circular queue of integers of user-specified
 * size using a simple array.
 *
 */
 ...
 * \brief A thread safe circular queue - a circular queue of integers of
 * user-specified size using a simple array. \param int The number of the
 * elements stored in the <code>circular_queue</code>. \note This is a sample
 */
class circular_queue {

  // Now that the overloaded operator function is using two variables, it cannot
  // be part of the class  You must declare it outside of the class.  In order
  // for the function to access private (and/or protected) member variables of
  // the class, you should make it a friend of the class See :
...
public:
   ...
   *  \throw This constructor throw an exception "Size value must be bigger than
   * 0" if the size is not correct \throw This constructor throw an exception if
   * it cannot create the array of integer (it can be std::bad_alloc,
   * std::out_of_range, ...)
   */
   ...
   *
   * \param reset This boolean reset the value to the default value
   * DEFAULT_VALUE after the value was retrieved. \return An integer that give
   ...
   * \note This method has been designed for test purposes and may access
   * inconsistent memory values (if the queue is not full and initialized)
   */
  ...
  // inline bool full(void) const  { return ( ((i_tail + 1) % (ui_size - 1)) ==
  // i_head ); }
};
