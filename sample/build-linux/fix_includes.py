...
_NAMESPACE_START_RE = re.compile(
    r'\s*(namespace\b[^{]*{\s*)+(//.*)?$|'
    r'\s*(U_NAMESPACE_BEGIN)|'
    r'\s*(HASH_NAMESPACE_DECLARATION_START)',
)
_NAMESPACE_END_RE = re.compile(
    r'\s*(})|'
    r'\s*(U_NAMESPACE_END)|'
...
class IWYUOutputRecord(object):
    ...
    def Merge(self, other):
        ...
        self.seen_forward_declare_lines.update(
            other.seen_forward_declare_lines,
        )
...
class IWYUOutputParser(object):
    ...
    def _ProcessOneLine(self, line):
        ...
        if self.current_section is not None:
            self.lines_by_section.setdefault(
    ...
    def ParseOneRecord(self, iwyu_output, flags):
        ...
        for line in iwyu_output:
            ...
            if not self._ProcessOneLine(line):
                break
        else:                                  # for/else
            return None                          # at EOF
        ...
        for line in self.lines_by_section.get(self._TOTAL_SECTION_RE, []):
            ...
            if m:
                if not flags.comments:
                    # pretend there were no comments
                    line = _COMMENT_RE.sub('', line)
                else:
...
def _CalculateMoveSpans(file_lines, forward_declare_spans):
    ...
    for line_number in range(len(file_lines)):
        if file_lines[line_number].type == _INCLUDE_RE:
            span_begin = _LineNumberStartingPrecedingComments(
...
def _CalculateReorderSpans(file_lines):
    ...
    while i < len(sorted_move_spans):
        ...
        for line_number in range(reorder_span_start, reorder_span_end):
            file_lines[line_number].reorder_span = (
                reorder_span_start,
                reorder_span_end,
            )
        i += 1
        ...
        while end_line < len(file_lines):
            ...
            if line_info.deleted:
                ...
                if namespace_depth <= 0:
                    ...
                    start_line = _LineNumberStartingPrecedingComments(
                        file_lines,
                        start_line,
                    )
    ...
    for line_range in line_ranges:
        for line_number in range(*line_range):
            if file_lines[line_number].type in (_BLANK_LINE_RE, _COMMENT_LINE_RE):
                continue
            uncommented_line = _COMMENT_RE.sub(
                '', file_lines[line_number].line,
            )
...
def _DeleteExtraneousBlankLines(file_lines, line_range):
    ...
    while (
        before_line and file_lines[before_line].type == _BLANK_LINE_RE and
...
def _ShouldInsertBlankLine(
    decorated_move_span, next_decorated_move_span,
    file_lines, flags,
):
    ...
    if decorated_move_span[0] != next_decorated_move_span[0]:
        next_line = _NextNondeletedLine(
            file_lines, decorated_move_span[0][1] - 1,
        )
    ...
    (this_kind, next_kind) = (
        decorated_move_span[1], next_decorated_move_span[1],
    )
...
def _GetFirstNamespaceLevelReorderSpan(file_lines):
    ...
    for line_number in range(len(file_lines)):
        ...
        if line_info.type in (
            _COMMENT_LINE_RE, _BLANK_LINE_RE, _INCLUDE_RE,
            _HEADER_GUARD_RE, _HEADER_GUARD_DEFINE_RE,
            _PRAGMA_ONCE_LINE_RE,
        ):
            continue
        ...
        elif line_info.type in (
            _IF_RE, _NAMESPACE_END_RE, _ELSE_RE, _ENDIF_RE,
            None,
        ):    # None is a 'normal' contentful line
...
def _FirstReorderSpanWith(
    file_lines, good_reorder_spans, kind, filename,
    flags,
):
    ...
    *before* the using line!
    ...
    """
    assert kind in (
        _MAIN_CU_INCLUDE_KIND, _C_SYSTEM_INCLUDE_KIND,
        _CXX_SYSTEM_INCLUDE_KIND, _NONSYSTEM_INCLUDE_KIND,
        _PROJECT_INCLUDE_KIND, _FORWARD_DECLARE_KIND,
    ), kind
    ...
    for reorder_span in good_reorder_spans:
        for line_number in range(*reorder_span):
            line_kind = _GetLineKind(
                file_lines[line_number], filename,
            ...
            if (
                line_kind == _FORWARD_DECLARE_KIND and
    ...
    while line_number < len(file_lines):
        if file_lines[line_number].deleted:
            ...
            file_lines[line_number].type == _COMMENT_LINE_RE
...
def _DecoratedMoveSpanLines(iwyu_record, file_lines, move_span_lines, flags):
    ...
    if m:
        ...
        # the iwyu version has comments
        if _COMMENT_LINE_RE.search(iwyu_version):
        ...
        contentful_list = [
            li.line for li in move_span_lines[first_contentful_line:]
            if not li.deleted
        ]
    ...
    kind = _GetLineKind(
        firstline, iwyu_record.filename,
        flags.separate_project_includes,
    )
    ...
    if reorder_span is None:     # must be a new #include we're adding
        ...
        if kind == _FORWARD_DECLARE_KIND:
            ...
            if (
                ...
                # succeeding.
                new_firstline = _RemoveNamespacePrefix(
                    firstline.line, namespace_prefix,
                )
                if new_firstline:
        ...
        if reorder_span is None:
            ...
            reorder_span = _FirstReorderSpanWith(
                file_lines, toplevel_reorder_spans,
...
def _NormalizeNamespaceForwardDeclareLines(lines):
    ...
    for line in lines + ['']:
        ...
        differ_pos = _CommonPrefixLength(
            namespaces_in_line, current_namespaces,
        )
        ...
        if namespaces_in_line:
            ...
            if not m:
                raise FixIncludesError(
                    'Malformed namespace line from iwyu: %s', line,
                )
...
def FixFileLines(iwyu_record, file_lines, flags):
    ...
    decorated_move_spans.append((
        (len(file_lines), len(file_lines)),
        _EOF_KIND, '', [],
    ))
...
def GetFixedFile(iwyu_record, flags):
    ...
    """
    file_contents = _ReadWriteableFile(
        iwyu_record.filename,
...
def CreateCLForCheckoutCommand(checkout_command, invoking_command):
    ...
    if not description_added:
        print(
            'ERROR: Didn\'t find "\t<enter description here>" in'
    ...
    output = _GetCommandOutputWithInput(
        '%s change -i'
        % what4, input_text,
    )
...
def main(argv):
    ...
    parser.add_option(
        '-b', '--blank_lines', action='store_true', default=True,
        help=(
            'Put a blank line between primary header file and'
    ...
    parser.add_option(
        '--noblank_lines',
        action='store_false', dest='blank_lines',
    )
    ...
    parser.add_option(
        '--safe_headers', action='store_true', default=True,
        help=(
    ...
    parser.add_option(
        '--nosafe_headers', action='store_false',
        dest='safe_headers',
    )

    parser.add_option(
        '-s', '--sort_only', action='store_true',
        help=(
            'Just sort #includes of files listed on cmdline;'
    ...
    parser.add_option(
        '--ignore_re', default=None,
        help=(
            'fix_includes.py will skip editing any file whose'
            ' name matches this regular expression.'
        ),
    )

    parser.add_option(
        '--checkout_command', default=None,
        help=(
            'A command, such as "p4 edit", to run on all the'
    ...
    parser.add_option(
        '--create_cl_if_possible', action='store_true',
        default=True,
        help=(
            'If --checkout_command is "p4|g4|v4 edit" and'
            ' all files to be modified needed to be checked out,'
            ' then create a CL containing those files.'
        ),
    )
    parser.add_option(
        '--nocreate_cl_if_possible', action='store_false',
        dest='create_cl_if_possible',
    )

    parser.add_option(
        '--append_to_cl', action='store',
        default=None, type='int', dest='append_to_cl',
        help=(
            'If provided, with a checkout_command, add files'
            ' that need fixing to the specified existing CL.'
        ),
    )

    parser.add_option(
        '--separate_project_includes', default=None,
        help=(
    ...
    parser.add_option(
        '--invoking_command_line', default=None,
        help=(
    ...
    parser.add_option(
        '-m', '--keep_iwyu_namespace_format', action='store_true',
        default=False,
        help=(
    ...
    parser.add_option(
        '--nokeep_iwyu_namespace_format', action='store_false',
    ...
    if flags.append_to_cl and flags.create_cl_if_possible:
        sys.exit(
            'FATAL ERROR: At most one of --append_to_cl and '
