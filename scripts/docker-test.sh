...
export DOCKER_NAME=${DOCKER_NAME:-"nabla-cpp-test"}
...
echo -e "docker run --rm --volume ${WORKING_DIR}:/mnt --volume /var/run/docker.sock:/var/run/docker.sock nabla/aquasec-scanner-cli:latest scan --user scanner --password password --host http://aqua:8080 --local --direct-cc --jsonfile /mnt/test.AquaSec --htmlfile /mnt/test nabla/nabla-cpp:latest"

echo -e "http://aqua:8080/#!/app/images/hub-docker/nabla/nabla-cpp-test/latest"
echo -e "docker run -e BUILD_JOB_NAME=NABLA-aqua/nabla-cpp/develop -e BUILD_URL=https://jenkins.nabla.mobi/jenkins/job/NABLA-aqua/job/nabla-cpp/job/develop/62/ -e BUILD_NUMBER=62 --rm -v /var/run/docker.sock:/var/run/docker.sock nabla/aquasec-scanner-cli:latest scan --host http://aqua:8080 --local hub.docker.com/nabla/nabla-cpp:latest --registry hub.docker.com --register --show-negligible --checkonly --no-verify --html --user scanner --password password"
